    ...
    for scene in nusc.scene:
        ...
        while has_more_frames:
            ...
            if os.getcwd() in lidar_path:
                ...
                # relative path
            if not bench2driveMMCV.is_filepath(lidar_path):
...
def _fill_trainval_infos(nusc,
    ...
    for sample in bench2driveMMCV.track_iter_progress(nusc.sample):
...
def export_2d_annotation(root_path, info_path, version, mono3d=False):
    ...
    nusc_infos = bench2driveMMCV.load(info_path)['infos']
    nusc = NuScenes(version=version, dataroot=root_path, verbose=True)
    # info_2d_list = []
    cat2Ids = [
    ...
    for info in bench2driveMMCV.track_iter_progress(nusc_infos):
        for cam in camera_types:
