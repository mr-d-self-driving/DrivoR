...
def get_img_annos(nuim, img_info, cat2id, out_dir, data_root, seg_root):
    ...
    bench2driveMMCV.imwrite(semseg_mask, seg_filename)
...
def export_nuim_to_coco(nuim, data_root, out_dir, extra_tag, version, nproc):
    ...
    for sample_info in bench2driveMMCV.track_iter_progress(nuim.sample_data):
    ...
    bench2driveMMCV.mkdir_or_exist(seg_root)
    bench2driveMMCV.mkdir_or_exist(osp.join(data_root, 'calibrated'))
    ...
    if nproc > 1:
        outputs = bench2driveMMCV.track_parallel_progress(
            process_img_anno, images, nproc=nproc)
    else:
        outputs = []
        for img_info in bench2driveMMCV.track_iter_progress(images):
