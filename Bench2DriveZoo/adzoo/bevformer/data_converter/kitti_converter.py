...
def _calculate_num_points_in_gt(data_path,
                                ...
                                remove_outside=True,
                                num_features=4):
    for info in bench2driveMMCV.track_iter_progress(infos):
...
def create_kitti_info_file(data_path,
    ...
    bench2driveMMCV.dump(kitti_infos_train, filename)
    ...
    bench2driveMMCV.dump(kitti_infos_val, filename)
    filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    print(f'Kitti info trainval file is saved to {filename}')
    bench2driveMMCV.dump(kitti_infos_train + kitti_infos_val, filename)
    ...
    bench2driveMMCV.dump(kitti_infos_test, filename)
...
def create_waymo_info_file(data_path,
    ...
    bench2driveMMCV.dump(waymo_infos_train, filename)
    #
    ...
    # bench2driveMMCV.dump(waymo_infos_val, filename)
    # filename = save_path / f'{pkl_prefix}_infos_trainval.pkl'
    # print(f'Waymo info trainval file is saved to {filename}')
    # bench2driveMMCV.dump(waymo_infos_train + waymo_infos_val, filename)
    ...
    # bench2driveMMCV.dump(waymo_infos_test, filename)
...
def _create_reduced_point_cloud(data_path,
    ...
    """
    kitti_infos = bench2driveMMCV.load(info_path)

    for info in bench2driveMMCV.track_iter_progress(kitti_infos):
        pc_info = info['point_cloud']
        image_info = info['image']
        calib = info['calib']
...
def export_2d_annotation(root_path, info_path, mono3d=True):
    ...
    for info in bench2driveMMCV.track_iter_progress(kitti_infos):
        ...
        (height, width,
         _) = bench2driveMMCV.imread(osp.join(root_path,
    ...
    bench2driveMMCV.dump(coco_2d_dict, f'{json_prefix}.coco.json')
