...
from bench2driveMMCV.datasets import LyftDataset
...
def create_lyft_infos(root_path,
    ...
    if version == 'v1.01-train':
        train_scenes = bench2driveMMCV.list_from_file('data/lyft/train.txt')
        val_scenes = bench2driveMMCV.list_from_file('data/lyft/val.txt')
    elif version == 'v1.01-test':
        train_scenes = bench2driveMMCV.list_from_file('data/lyft/test.txt')
        val_scenes = []
    else:
    ...
    if test:
        ...
        bench2driveMMCV.dump(data, info_path)
    else:
        ...
        bench2driveMMCV.dump(data, info_val_path)
...
def _fill_trainval_infos(lyft,
    ...
    for sample in bench2driveMMCV.track_iter_progress(lyft.sample):
...
def export_2d_annotation(root_path, info_path, version):
    ...
    lyft_infos = bench2driveMMCV.load(info_path)['infos']
    lyft = Lyft(
        data_path=osp.join(root_path, version),
        json_path=osp.join(root_path, version, version),
        verbose=True)
    ...
    for info in bench2driveMMCV.track_iter_progress(lyft_infos):
        for cam in camera_types:
            ...
            (height, width, _) = bench2driveMMCV.imread(cam_info['data_path']).shape
            ...
            for coco_info in coco_infos:
                ...
                coco_ann_id += 1
    bench2driveMMCV.dump(coco_2d_dict, f'{info_path[:-4]}.coco.json')
