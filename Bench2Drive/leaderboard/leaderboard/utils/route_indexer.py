    ...
    def validate_and_resume(self, endpoint):
        ...
        while check_index < resume_index:
            try:
                ...
                if route_data[check_index]['status'] not in ['Failed', 'Failed - Simulation crashed', 'Failed - Agent crashed', 'Failed - Simulation crashed', "Failed - Agent couldn't be set up"]:
                    check_index += 1
                else:
